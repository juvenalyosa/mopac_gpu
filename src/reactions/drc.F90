...
!
      ...
      logical :: addk, letot, let, velred, opend, parmax, debug, l_debug = .false., l_irc, &
        l_dipole, l_pdb
!
!  TEMPERATURE=n: starting velocities from the Maxwell-Boltzmann distribution at n K;
!  BUSSI=n: stochastic velocity rescaling thermostat at that temperature, time constant
!  n fs (see drc_ensemble.F90)
!
      logical :: l_bussi, l_temp
      integer :: n_dof, n_steps
      double precision :: t_target, tau_fs, e_bath, t_sum, t_inst, sim_fs
      double precision, parameter :: kb_kcal = 0.0019872041d0   ! kcal/(mol K)
      double precision, external :: ddot, dipole, reada, seconds, drc_bussi_factor
!***********************************************************************
!                                                                      *
...
!
      ...
      l_temp = (index(keywrd, " TEMPERATURE=") /= 0 .and. .not. l_irc)
      l_bussi = (index(keywrd, " BUSSI") /= 0 .and. l_temp)
      t_target = 0.d0
      if (l_temp) t_target = reada(keywrd, index(keywrd, " TEMPERATURE="))
      tau_fs = 100.d0
      if (index(keywrd, " BUSSI=") /= 0) tau_fs = max(1.d0, reada(keywrd, index(keywrd, " BUSSI=")))
      n_dof = max(1, 3*numat - 3)
      e_bath = 0.d0
      t_sum = 0.d0
      sim_fs = 0.d0
      n_steps = 0
      if (l_temp) call drc_seed_random(keywrd)
      if (l_temp .and. t_target > 0.d0 .and. .not. velred .and. index(keywrd, "RESTART") == 0) then
!
!  Maxwell-Boltzmann velocities, then handled exactly as velocities supplied by VELOCITY
!
        call drc_maxwell_boltzmann(startv, t_target, numat, atmass, n_dof)
        velred = .true.
        write (iw,'(/10x,a,f9.2,a,i8,a)') "STARTING VELOCITIES FROM THE MAXWELL-BOLTZMANN DISTRIBUTION AT", &
          t_target, " K,", n_dof, " DEGREES OF FREEDOM"
      end if
      if (l_bussi) write (iw,'(10x,a,f9.2,a,f8.1,a)') "BUSSI THERMOSTAT AT", t_target, &
        " K, TIME CONSTANT", tau_fs, " FEMTOSECONDS"
...
!
        ...
        if (l_bussi .and. t_target > 0.d0 .and. ekin > 1.d-10 .and. iloop > 1) then
!
!  Bussi thermostat.  The energy exchanged with the bath is added to ETOT, so ERROR in the
!  DRC output remains the error of the integration.
!
        ...
        if (l_temp) then
          ...
          n_steps = n_steps + 1
          t_sum = t_sum + t_inst
          if (mod(n_steps, 50) == 0) write (iw,'(10x,a,i8,a,f10.3,a,f9.2,a,f9.2,a,f12.4)') &
            "TEMPERATURE: STEP", n_steps, "  TIME(FS)", sim_fs, "  T(K)", t_inst, &
            "  <T>(K)", t_sum/n_steps, "  ENERGY TO BATH (KCAL/MOL)", e_bath
        end if
!
