

                     SUMMARY OF  PM7 CALCULATION

                                                       MOPAC v23.1.2 MacOS
                                                       Wed Oct  1 13:12:25 2025

           Empirical Formula: H2 O  =     3 atoms

 PM7 XYZ 1SCF PULAY BONDS
 Water single point (force GPU via env MOPAC_FORCEGPU=1)



     1SCF WAS SPECIFIED, SO BFGS WAS NOT USED                 
     SCF FIELD WAS ACHIEVED                                   

          HEAT OF FORMATION       =        -57.79067 KCAL/MOL =    -241.79617 KJ/MOL
          DIPOLE                  =          2.14029 DEBYE   POINT GROUP:  C2v 
          NO. OF FILLED LEVELS    =          4
          IONIZATION POTENTIAL    =         12.096225 EV
          HOMO LUMO ENERGIES (EV) =        -12.096  4.045
          MOLECULAR WEIGHT        =         18.0152
          COSMO AREA              =         42.49 SQUARE ANGSTROMS
          COSMO VOLUME            =         25.20 CUBIC ANGSTROMS

          MOLECULAR DIMENSIONS (Angstroms)

            Atom       Atom       Distance
            H     3    H     2     1.51432
            H     2    O     1     0.58626
            O     1    H     2     0.00000
          SCF CALCULATIONS        =          1
          WALL-CLOCK TIME         =      0.004 SECONDS
          COMPUTATION TIME        =      0.003 SECONDS


          FINAL GEOMETRY OBTAINED
 PM7 XYZ 1SCF PULAY BONDS
 Water single point (force GPU via env MOPAC_FORCEGPU=1)

  O     0.00000000 +1   0.00000000 +1   0.00000000 +1
  H     0.00000000 +1   0.75716000 +1   0.58626000 +1
  H     0.00000000 +1  -0.75716000 +1   0.58626000 +1

