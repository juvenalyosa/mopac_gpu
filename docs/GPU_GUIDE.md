...
- Point-charge/dipole pairs of the sparse Fock build (all atom pairs beyond the NDDO cutoff, ~22 million for a 6700-atom protein) no longer run as one 32-thread block per pair: a grid-stride kernel accumulates per-atom scalars (monopole term on the whole diagonal, dipole term on the first four diagonal elements and on the s-p elements) with a warp-segmented reduction on the row side and one atomic per pair on the column side, and a per-atom kernel applies them to F. Same arithmetic as before; summation order differs (already non-deterministic through the atomics).
- After a successful device pack of the resident Fock plan the default path trusts the kernel counters, as the strict path always did; the host re-count (`count_resident_plan`, an O(numat^2) scan of `nijbo`, 0.55 s per geometry step at 7000 atoms) only runs with `MOPAC_GPU_PROFILE` set, reported as `fock_plan_count_host`.
- The resident SCF device context is kept alive across SCF calls of a job (geometry optimization, MD): `mopac_cuda_mozyme_scf_setup` re-initialises the existing context in place and the device buffers are reused when their sizes do not change, instead of freeing and re-allocating every buffer per step; it is released at the end of the job (`mozyme_gpu_scf_release_context`) or on any failure path. `scf_try_prepare` in the section report covers config/state registration.
- `deriv` no longer builds the finite-difference Jacobian dCartesian/dParameter when every atom is Cartesian (no dummy atoms, no symmetry relations, molecular system, no SADDLE): the parameter gradient is copied straight from `dxyz`. The old construction (`jcarin` + `mxm`, O(nvar x 3 numat) with one cuBLAS call per two variables on GPU builds) cost 8 s per geometry step for a 6700-atom protein and 30 ms for crambin; results differ from it only by the finite-difference rounding (~1e-6 kcal/mol on the heat after three cycles). Internal coordinates keep the original path.
