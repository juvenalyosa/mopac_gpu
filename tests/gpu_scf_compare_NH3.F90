program gpu_scf_compare_NH3
  ...
  sys%coord = [ 0.0000d0, 0.0000d0, 0.0000d0, &
                0.0000d0, 0.0000d0, 1.012d0, &
                0.942d0, 0.0000d0, -0.337d0, &
               -0.471d0, 0.816d0, -0.337d0 ]
  sys%model = 0
  sys%epsilon = 1.0d0
  sys%spin = 0
  sys%tolerance = 1.0d0
  ...
  if (st_cpu%mpack <= 0) then
    print *, 'CPU state not available (NH3)'
    stop 1
  end if
  ...
  if (st_gpu%mpack <= 0) then
    print *, 'GPU state not available (NH3)'
    stop 1
  end if
  ...
  print *, 'NH3 SCF density max abs diff:', d_pa
  print *, 'NH3 SCF density rel diff:', d_pa/denom_pa
  call scf_assert('NH3_SCF', d_pa, denom_pa)

  n = min(size(f_cpu), size(f_gpu))
  d_f = maxval(abs(f_cpu(:n) - f_gpu(:n)))
  denom_f = max(1.0d0, maxval(abs(f_cpu(:n))))
  print *, 'NH3 Fock max abs diff:', d_f
  print *, 'NH3 Fock rel diff:', d_f/denom_f
  call scf_assert_fock('NH3_FOCK', d_f, denom_f)
...
end program gpu_scf_compare_NH3
