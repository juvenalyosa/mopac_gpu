program gpu_grad_compare_HO
  ...
  ! H-O diatomic (single-heavy case)
  sys%natom = 2
  sys%natom_move = 2
  allocate(sys%atom(2))
  sys%atom = [1, 8]
  allocate(sys%coord(6))
  sys%coord = [ -0.5d0, 0.0d0, 0.0d0, &
                 0.5d0, 0.0d0, 0.0d0 ]
  sys%model = 0   ! PM7
  sys%epsilon = 1.0d0
  sys%spin = 0
  ...
  print *, 'HO Grad max abs diff:', diff
  print *, 'HO Grad rel diff:', diff/denom
  call grad_assert('HO', diff, denom)
contains
...
end program gpu_grad_compare_HO
