program gpu_scf_compare_NO_UHF
  use mopac_api_f
  ...
  if (st_cpu%mpack <= 0) then
    print *, 'CPU state not available (NO UHF)'
    stop 1
  end if
  ...
  if (st_gpu%mpack <= 0) then
    print *, 'GPU state not available (NO UHF)'
    stop 1
  end if
  ...
  print *, 'NO(UHF) SCF density max abs diff:', d_pa
  print *, 'NO(UHF) SCF density rel diff:', d_pa/denom_pa
  call scf_assert('NO_UHF_SCF', d_pa, denom_pa)

  n = min(size(f_cpu), size(f_gpu))
  d_f = maxval(abs(f_cpu(:n) - f_gpu(:n)))
  denom_f = max(1.0d0, maxval(abs(f_cpu(:n))))
  print *, 'NO(UHF) Fock max abs diff:', d_f
  print *, 'NO(UHF) Fock rel diff:', d_f/denom_f
  call scf_assert_fock('NO_UHF_FOCK', d_f, denom_f)
...
end program gpu_scf_compare_NO_UHF
