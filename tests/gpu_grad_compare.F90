program gpu_grad_compare
  ...
  ! Small H2 system (all light-light pairs) to exercise GPU dfock2 LL path
  sys%natom = 2
  sys%natom_move = 2
  allocate(sys%atom(2))
  sys%atom = [1, 1]
  allocate(sys%coord(6))
  sys%coord = [ -0.37d0, 0.0d0, 0.0d0, &
                 0.37d0, 0.0d0, 0.0d0 ]
  sys%model = 0   ! PM7
  sys%epsilon = 1.0d0
  sys%spin = 0
  ...
  ! CPU gradient via relax API (one evaluation)
#ifdef GPU
  lgpu = .false.
#endif
  ...
  print *, 'Grad max abs diff:', diff
  print *, 'Grad rel diff:', diff/denom
  call grad_assert('H2', diff, denom)
contains
