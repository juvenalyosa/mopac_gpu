program gpu_grad_compare_NH3
  ...
  if (.not. allocated(props%coord_deriv)) then
    print *, 'No gradient from CPU relax (NH3)'
    stop 1
  end if
  ...
  call mopac_relax_f(sys, st, props)
  if (.not. allocated(props%coord_deriv)) then
    print *, 'No gradient from GPU relax (NH3)'
    stop 1
  end if
  ...
  print *, 'NH3 Grad max abs diff:', diff
  print *, 'NH3 Grad rel diff:', diff/denom
  call grad_assert('NH3', diff, denom)
contains
...
end program gpu_grad_compare_NH3
