program gpu_grad_compare_NO_UHF
  use mopac_api_f
#ifdef GPU
  ...
  if (.not. allocated(props%coord_deriv)) then
    print *, 'No gradient from CPU relax (NO UHF)'
    stop 1
  end if
  ...
  call mopac_relax_f(sys, st, props)
  if (.not. allocated(props%coord_deriv)) then
    print *, 'No gradient from GPU relax (NO UHF)'
    stop 1
  end if
  ...
  print *, 'NO(UHF) Grad max abs diff:', diff
  print *, 'NO(UHF) Grad rel diff:', diff/denom
  call grad_assert('NO(UHF)', diff, denom)
contains
...
end program gpu_grad_compare_NO_UHF
